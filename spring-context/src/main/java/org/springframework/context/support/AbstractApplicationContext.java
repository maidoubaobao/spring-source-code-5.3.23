...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	protected ConfigurableEnvironment createEnvironment() {
		/*
			这里创建了环境类，并初始化了属性资源和属性解析器两个属性： propertySources/propertyResolver
			将环境变量和系统变量封装到了 propertySources 中
		 */
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			...
			/*
			做准备工作，校验必需的属性、初始化监听器和事件集合
			 */
			// Prepare this context for refreshing.
			prepareRefresh();

			/*
			创建bean工厂，解析配置文件，将bean定义信息注册到bean工厂中
			 */
			// Tell the subclass to refresh the internal bean factory.
			ConfigurableListableBeanFactory beanFactory = obtainFreshBeanFactory();

			/*
			准备bean工厂，主要是设置了一些属性，如加载器、处理器和资源类等
			 */
			...
			try {
				...
				/*
				调用bean工厂后置处理器，包括 BeanDefinitionRegistryPostProcessor.postProcessBeanDefinitionRegistry()/postProcessBeanFactory() 和
				...
				/*
				注册bean后置处理器，使用<context:component-scan>标签时，会注册 AutowiredAnnotationBeanPostProcessor
				除此之外，还将 BeanPostProcessorChecker 注册进去了，重新注册了 ApplicationListenerDetector
				 */
				...
				/*
				初始化消息源，一般情况下，bean工厂里找不到，就设置一个默认的消息源对象：DelegatingMessageSource
				 */
				// Initialize message source for this context.
				initMessageSource();

				/*
				初始化应用事件组播器，一般情况下，bean工厂中找不到，设置一个默认的组播器：SimpleApplicationEventMulticaster
				 */
				...
				/*
				将事件监听器注册到组播器中，默认情况下，没有监听器，也没有要发布的早期事件
				 */
	...
	protected ConfigurableListableBeanFactory obtainFreshBeanFactory() {
		/*
			这里进入的是 AbstractRefreshableApplicationContext 的方法
			这里是创建了bean工厂，并解析配置文件，将bean定义信息注册到bean工厂中
		 */
		refreshBeanFactory();
		/*
			将创建的bean工厂返回
	...
	protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
		...
		if (!shouldIgnoreSpel) {
			/*
				这个是Spel表达式解析器，这个解析器很强大能解析很多表达式
				能将字符串解析成基础类型（如Integer/Boolean），能将字符串解析成Class，还支持解析#{xxx}等等
			 */
	...
	protected void initMessageSource() {
		/*
		初始化消息源，一般情况下，bean工厂里找不到，就设置一个默认的消息源对象
		 */
		...
		if (beanFactory.containsLocalBean(MESSAGE_SOURCE_BEAN_NAME)) {
			...
			/*
			一般情况下，bean工厂中是没有消息源对象的，会走到这里，设置一个空的默认的消息源
			 */
			// Use empty MessageSource to be able to accept getMessage calls.
			DelegatingMessageSource dms = new DelegatingMessageSource();
			/*
			getInternalParentMessageSource() 返回的是null，取不到父消息源
			 */
