...
public abstract class AbstractEnvironment implements ConfigurableEnvironment {
	...
	protected AbstractEnvironment(MutablePropertySources propertySources) {
		...
		/*
			这里创建了 PropertySourcesPropertyResolver 类
			将 MutablePropertySources 类设置到 propertySources 属性中
		 */
		this.propertyResolver = createPropertyResolver(propertySources);
		/*
			这里调用了子类 StandardEnvironment 的实现
			将系统变量 PropertiesPropertySource 和环境变量 SystemEnvironmentPropertySource 放到 MutablePropertySources 类中
		 */
	...
	public void validateRequiredProperties() throws MissingRequiredPropertiesException {
		/*
			通过标准环境对象 StandardEnvironment 创建的解析器 propertyResolver 是 PropertySourcesPropertyResolver
			继承自 AbstractPropertyResolver
			这里是校验是否有必需的属性，如果必需的属性没传会报错不让启动，必需的属性可以通过上面的 setRequiredProperties() 方法设置
			属性可以设置到环境变量中
		 */
