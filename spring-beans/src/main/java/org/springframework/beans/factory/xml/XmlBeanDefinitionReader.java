...
public class XmlBeanDefinitionReader extends AbstractBeanDefinitionReader {
	...
	public int loadBeanDefinitions(Resource resource) throws BeanDefinitionStoreException {
		/*
			这里的 resource 是 ClassPathContextResource 对象，给它封装成了编码资源
		 */
	...
	public int loadBeanDefinitions(EncodedResource encodedResource) throws BeanDefinitionStoreException {
		...
		/*
			这是资源类集合，这里是空的，下面把启动配置文件的资源类放进去了
		 */
		...
		/*
			这里的 encodedResource.getResource() 是 ClassPathContextResource，继承自 ClassPathResource
			getInputStream() 在 ClassPathResource 类中，实现原理是通过 classLoader.getResourceAsStream(path) 方法来获取文件流

			try() 里直接获取文件流，这是java的一个语法糖，java会在 try 执行完后自动关闭流，try 里支持写多行代码
		 */
		try (InputStream inputStream = encodedResource.getResource().getInputStream()) {
			...
			/*
				这里将配置文件流和资源对象传进去解析，解析出所有的bean定义信息并注册到bean工厂中
			 */
	...
	protected int doLoadBeanDefinitions(InputSource inputSource, Resource resource)
			throws BeanDefinitionStoreException {
		/*
			入参中的 resource 是 ClassPathContextResource 对象
		 */
		try {
			/*
				这里是将配置文件流解析成文档对象，这里就不细看了
			 */
			Document doc = doLoadDocument(inputSource, resource);

			/*
				这里就是解析配置文件的内容了，解析了所有的标签，并将bean定义信息全部注册到bean工厂中
			 */
	...
	public int registerBeanDefinitions(Document doc, Resource resource) throws BeanDefinitionStoreException {
		/*
			这里用反射创建了一个 DefaultBeanDefinitionDocumentReader 对象
		 */
		BeanDefinitionDocumentReader documentReader = createBeanDefinitionDocumentReader();
		/*
			getRegistry() 方法拿到的是Bean工厂对象 DefaultListableBeanFactory
			这里获取Bean工厂里Bean定义信息的数量，首次启动肯定是0
		 */
		int countBefore = getRegistry().getBeanDefinitionCount();

		/*
			这里是解析配置文件的核心逻辑，会解析所有的标签
			从配置文件里解析Bean定义信息，并注册到Bean工厂里去
			createReaderContext() 方法获取到的是 XmlReaderContext 对象
		 */
	...
	public XmlReaderContext createReaderContext(Resource resource) {
		/*
			getNamespaceHandlerResolver() 方法是初始化命名空间的处理器，处理器的配置位于 META-INF/spring.handlers
		 */
